                    ...
                    <div class="am-u-sm-12 am-u-md-12 am-u-lg-12">
                        <div class="widget am-cf">
                            ...
                            <div class="widget-body  am-fr">
                                    ...
                                    <div class="am-form-group">
                                        <div class="am-btn-toolbar">
                                            <div class="am-btn-group am-btn-group-xs">
                                                <a href="${ctxroot}/associatedgenes/toaddassociatedgenes">
                                ...
                                <div class="am-btn-group am-btn-group-xs">
                                    <a href="${ctxroot}/static/example/associatedgenes_example.csv">
                                        <button type="button" class="am-btn am-btn-default am-btn-success"> 导入关联基因样例文件
                                        </button>
                                    </a>
                                </div>
                                <div class="am-u-sm-12">
                                    <table width="100%" class="am-table am-table-striped am-table-bordered am-table-compact" id="example-r">
                                        ...
                                        <tbody>
                                        <%--<c:forEach items="${associatedgenes}" var="item">--%>
                                            <%--<tr>--%>
                                                <%--<td>${item.id}</td>--%>
                                                <%--<td>${item.qtlName}</td>--%>
                                                <%--<td>${item.version}</td>--%>
                                                <%--<td>${item.associatedGenes}</td>--%>
                                                <%--<td>--%>
                                                    <%--<div class="tpl-table-black-operation">--%>
                                                        <%--<a href="${ctxroot}/associatedgenes/toedit?id=${item.id}">--%>
                                                            <%--<i class="am-icon-pencil"></i> 编辑--%>
                                                        <%--</a>--%>
                                                        <%--<a href="${ctxroot}/associatedgenes/delete?id=${item.id}" class="tpl-table-black-operation-del">--%>
...
<script>
    $(function(){
        $('#example-r').DataTable({
                    ...
                    "sAjaxSource": "${ctxroot}/associatedgenes/index",//通过ajax实现分页的url路径。
                    "columns": [
                        {"data": "id"},
                        {'data': 'qtlName'},
                        {'data': 'version'},
                        ...
                        {render:function(data,type,row){
                            return  "<div class='tpl-table-black-operation'><a href='${ctxroot}/associatedgenes/toedit?id="+row.id+"'><i class='am-icon-pencil'></i> 编辑</a>"+"<a class='tpl-table-black-operation-del del-btn' href='javascript:void(0);' data-id='"+row.id+"'><i class='am-icon-trash'></i> 删除</a></div>";
                        }}
                    ],
                    "columnDefs":[{
                        "targets": 3,
    ...
    $("body").on("click", ".del-btn", function() {
        ...
        $('#my-confirm').modal({
            ...
            onConfirm: function(options) {
                // 删除
                $.ajax({
                    url: "${ctxroot}/associatedgenes/delete",
                    data: {id: id},
                    type:"GET",
                    dataType: "text",
