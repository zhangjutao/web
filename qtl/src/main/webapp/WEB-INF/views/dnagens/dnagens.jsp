...
<script>
    $(function () {
        $('#example-r').DataTable({
                    ...
                    "sAjaxSource": "${ctxroot}/dnagens/index",//通过ajax实现分页的url路径。
                    "columns": [
                        {"data": "id"},
                        {'data': 'gene'},
                        {'data': 'geneName'},
                        {'data': 'geneFunction'},
                        {
                            render: function (data, type, row) {
                                return "<div class='tpl-table-black-operation'><a href='${ctxroot}/dnagens/toedit?id=" + row.id + "'><i class='am-icon-pencil'></i> 编辑</a>" + "<a class='tpl-table-black-operation-del del-btn' href='javascript:void(0)' data-id='" + row.id + "'><i class='am-icon-trash'></i> 删除</a></div>";
                            }
                        }
                    ],
                    "bAutoWidth": true, //自适应宽度
