                ...
                <div class="am-u-sm-12 am-u-md-12 am-u-lg-12">
                    <div class="widget am-cf">
                        ...
                        <div class="widget-body  am-fr">
                            ...
                            <div class="am-btn-group am-btn-group-xs">
                                <a href="${ctxroot}/static/example/qtlref_example.csv">
                                    <button type="button" class="am-btn am-btn-default am-btn-success"> 导入QTLref样例文件
                                    </button>
                                </a>
                            </div>
                            <div class="am-u-sm-12">
                                <table width="100%" class="am-table am-table-striped am-table-bordered am-table-compact"
                                    ...
                                    <tbody>
                                    <%--<c:forEach items="${qtlref}" var="item">--%>
                                    <%--<tr>--%>
                                    <%--<td>${item.id}</td>--%>
                                    <%--<td>${item.title}</td>--%>
                                    <%--<td>${item.authors}</td>--%>
                                    ...
                                    <%--<a href="${ctxroot}/qtlref/toedit?id=${item.id}">--%>
                                    <%--<i class="am-icon-pencil"></i> 编辑--%>
                                    <%--</a>--%>
                                    <%--<a href="javascript:;" class="tpl-table-black-operation-del btn-refuse"--%>
                                    <%--data-src="${ctxroot}/qtlref/delete?id=${item.id}">--%>
...
<script>
    $(function () {
        $('#example-r').DataTable({
            ...
            "sAjaxSource": "${ctxroot}/qtlref/index",//通过ajax实现分页的url路径。
            "columns": [
                {"data": "id"},
                {'data': 'qtlName'},
                {'data': 'title'},
                ...
                {'data': 'pubmed'},
                {
                    render: function (data, type, row) {
                        return "<div class='tpl-table-black-operation'><a href='${ctxroot}/qtlref/toedit?id=" + row.id + "'><i class='am-icon-pencil'></i> 编辑</a>" + "<a class='tpl-table-black-operation-del del-btn' href='javascript:void(0);' data-id='" + row.id + "'><i class='am-icon-trash'></i> 删除</a></div>";
                    }
                }
            ],
            "bAutoWidth": true, //自适应宽度
