...
//		sb.append(" 每页显示 <input class=\"form-control inline-block\" style=\"width:50px;\" type=\"text\" value=\""+pageSize+"\" onkeypress=\"PressPage(event,"+ pageNo + ",this.value, '"+funcParam+"')\"");
//		sb.append(" onclick=\"this.select();\"/> /页  ");
		...
		sb.append("<li class=\"disabled controls\"><a href=\"javascript:void(0);\">跳转到 ");
		sb.append("<input type=\"text\" value=\""+pageNo+"\" onkeypress=\"PressPage(event, this.value,"+ pageSize +", '" + funcParam + "')\"");
		sb.append(" onclick=\"this.select();\"/>页</a> <li>");

//		sb.append("共 " + count + " 条"+(message!=null?message:"")+"</a></li>\n");

        sb.append("<li class=\"disabled controls\"> 展示数量 <select value=\""+pageSize+"\" onchange=\"SelectPage(event," + pageNo + ",this.value, '"+funcParam+"')\" class=\"show-number\">");
        if(pageSize == 10) {
            sb.append("<option value=\"10\" selected=\"selected\">10条</option><option value=\"20\">20条</option><option value=\"50\">50条</option></select> /页</li> ");
        }
        if(pageSize == 20) {
            sb.append("<option value=\"10\">10条</option><option value=\"20\" selected=\"selected\">20条</option><option value=\"50\">50条</option></select> /页</li> ");
        }
        if(pageSize == 50) {
            sb.append("<option value=\"10\">10条</option><option value=\"20\">20条</option><option value=\"50\" selected=\"selected\">50条</option></select> /页</li> ");
        }
